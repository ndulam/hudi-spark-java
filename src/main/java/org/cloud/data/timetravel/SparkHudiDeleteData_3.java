...
public class SparkHudiDeleteData_3 {
    public static void main(String[] args) {
        ...
        Dataset trips_df = spark.read().format("org.apache.hudi")
                .load("file:///D:/sparksetup/sparkdata/trips_table_timetravel");

        Dataset trips_df_updates =  trips_df.filter(trips_df.col("rider").equalTo("rider-F"));
        trips_df_updates.write().format("org.apache.hudi").option("hoodie.datasource.write.operation","delete")
                .option(PARTITIONPATH_FIELD_NAME.key(),"city").option("hoodie.table.name","trips_table_timetravel").mode(SaveMode.Append)
